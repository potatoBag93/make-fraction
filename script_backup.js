...
class FractionGame {
    ...
    bindEvents() {
        ...
        if (startSimpleBtn) {
            ...
            console.error('start-simple-btn 요소를 찾을 수 없습니다');
        }
        
        if (openSettingsBtn) {
            openSettingsBtn.addEventListener('click', () => {
                console.log('상세설정 버튼 클릭됨');
                // 시작화면 값 -> settings에 반영
                const diff = document.getElementById('start-difficulty').value;
                const time = parseInt(document.getElementById('start-time-limit').value, 10);
            ...
            console.error('open-settings-btn 요소를 찾을 수 없습니다');
        }
        ...
        if (document.getElementById('help-btn')) document.getElementById('help-btn').addEventListener('click', () => this.showTutorial());
        if (document.getElementById('clear-storage-btn')) document.getElementById('clear-storage-btn').addEventListener('click', () => this.confirmClearStorage());
    ...
    loadSettings() {
        ...
        this.settings.totalQuestions = parseInt(document.getElementById('total-questions').value, 10);
        this.settings.skipAllowed = document.getElementById('skip-allowed').value === 'true';
        this.settings.skipCount = parseInt(document.getElementById('skip-count').value, 10);
        this.settings.hideAnswers = document.getElementById('hide-answers').value === 'true';
        
        // 대분수 설정값 읽기 및 유효성 검사
        this.settings.mixedNumberRatio = Math.max(0, Math.min(100, parseInt(document.getElementById('mixed-number-ratio').value, 10) || 30));
        this.settings.mixedNumberMax = Math.max(1, Math.min(10, parseInt(document.getElementById('mixed-number-max').value, 10) || 3));
